...
{   
   ...
   //int pathHTMLLength = strlen(path) + 10 + 1;
    char* indexHTML = malloc(strlen(path) + 10 + 1); //original path + index.html(10 chars) + NULL
    strcpy(indexHTML, path);
    strncat(indexHTML, "index.html", 10);
    
   // int pathPhpLength = strlen(path) + 9 + 1;
    char* indexPHP = malloc(strlen(path) + 9 + 1); //original path + "index.php(9 chars)" + NULL
    strcpy(indexPHP, path);
    strncat(indexPHP, "index.php", 9);
...
const char* lookup(const char* path)
{
    //find where "." starts in the path
    char* ptr = strrchr(path, '.');
    
    if(strncasecmp(ptr, ".css", 4) == 0) {
        return "text/css";
    }
    else if(strncasecmp(ptr, ".html", 5) == 0) {
        return "text/html";
    }
    else if(strncasecmp(ptr, ".gif", 4) == 0) {
        return "image/gif";
    }
    else if(strncasecmp(ptr, ".ico", 4) == 0) {
        return "image/x-icon";
    }
    else if(strncasecmp(ptr, ".jpg", 4) == 0) {
        return "image/jpeg";
    }
    else if(strncasecmp(ptr, ".js", 3) == 0) {
        return "text/javascript";
    }
    else if(strncasecmp(ptr, ".php", 4 ) == 0) {
        return "text/x-php";
    }
    else if(strncasecmp(ptr, ".png", 4) == 0) {
        return "image/png";
    }
    else {
        return NULL;
...
bool parse(const char* line, char* abs_path, char* query) {
    ...
    char method[4];
    int methodLength = (int)(strlen(line) - strlen(firstSpace));
    method[methodLength] = '\0';
    strncpy(method, line, methodLength);
    //if method is not GET, 405 bad request
    if(strncasecmp(method, "GET", methodLength) != 0) {
        error(405);
        return false;
    }
    ...
    char* questionMark = strchr(requestTarget, '?'); // string of words after ? (including)
    ...
    if(questionMark == NULL) {
        ...
        int absPathLength = (int) (requestTargetLength - strlen(questionMark));
        abs_path[absPathLength] = '\0';
        strncpy(abs_path, requestTarget, absPathLength);
        //store everything to right of question mark in query
        if ((questionMark + 1) != NULL) {
            int queryLength = (int) (strlen(questionMark) - requestTarget[requestTargetLength]);
            query[queryLength] = '\0';
